...
public class ProcessChartModel
{
    ...
    public ViewResolvingPlotModel ChartView { get; }

    public ProcessChartModel()
    {
        ChartView = new ViewResolvingPlotModel
        {
    ...
    public void SetFrameworkElement(FrameworkElement? el, int i)
    {
        UpdateTitle(el, i);
        UpdateColor(el);
    }

    public void UpdateTitle(FrameworkElement? element, int i)
    {
        if (element != null)
        {
            ...
            if (element.TryFindResource("爐內溫度1") is string s0)
            {
                LineSeries0.Title = s0;
            }

            if (element.TryFindResource("爐內溫度2") is string s2)
            {
            ...
            if (i == 2)
            {
                ...
                if (element.TryFindResource("爐內溫度3") is string s3)
                {
                    LineSeries2.Title = s3;
                }

                if (element.TryFindResource("爐內溫度4") is string s4)
                {
                    LineSeries3.Title = s4;
                }

                if (element.TryFindResource("爐內溫度5") is string s5)
                {
                    LineSeries4.Title = s5;
                }

                if (element.TryFindResource("爐內溫度6") is string s6)
                {
                    LineSeries5.Title = s6;
                }

                if (element.TryFindResource("爐內溫度7") is string s7)
                {
                    LineSeries6.Title = s7;
                }

                if (element.TryFindResource("爐內溫度8") is string s8)
                {
    ...
    public void AddData(RecordTemperatures tp, int i)
    {
        var t = DateTimeAxis.ToDouble(tp.AddedTime);
        LineSeries0.Points.Add(new DataPoint(t, tp.OvenTemperatures_1));
        LineSeries1.Points.Add(new DataPoint(t, tp.OvenTemperatures_2));
        if (i > 0)
        {
            LineSeries2.Points.Add(new DataPoint(t, tp.OvenTemperatures_3));
            LineSeries3.Points.Add(new DataPoint(t, tp.OvenTemperatures_4));
            LineSeries4.Points.Add(new DataPoint(t, tp.OvenTemperatures_5));
            LineSeries5.Points.Add(new DataPoint(t, tp.OvenTemperatures_6));
            LineSeries6.Points.Add(new DataPoint(t, tp.OvenTemperatures_7));
            LineSeries7.Points.Add(new DataPoint(t, tp.OvenTemperatures_8));
        }
